  ...
  <div>
    ...
    <v-row v-if="localSelected">
      ...
      <v-col cols="12" md="12">
        ...
        <v-file-input v-model="transcript5" outlined placeholder="File input" multiple show-size
      ...
      <v-col cols="12" md="12">
        ...
        <v-file-input v-model="showing_educational_qualifications5" outlined placeholder="File input" multiple show-size
          :accept="['image/jpeg', 'image/png', 'application/pdf']" @change="onFileChange"></v-file-input>
        <div style="text-align: end; margin-top: -20px;">
          หมายเหตุ : ขอให้ท่านบันทึกไฟล์เป็นนามสกุล .jpg .png .jpeg และ .pdf
        </div>
...
export default {
  data() {
    return {
      ...
      transcript5: [],
      showing_educational_qualifications5: [],
      daygraduation: null,
  ...
  watch: {
    ...
    transcript5(newVal) {
      if (newVal instanceof File) {
        this.formData.transcript5 = newVal; // เก็บไฟล์ทั้งก้อน
      } else if (Array.isArray(newVal) && newVal.length > 0) {
        this.formData.transcript5 = newVal[0]; // เก็บไฟล์แรก (ถ้ามีหลายไฟล์)
      } else {
        this.formData.transcript5 = null;
      }
      console.log("ข้อมูลไฟล์:", this.formData.transcript5);
    },
    showing_educational_qualifications5(newVal) {
      if (newVal instanceof File) {
        this.formData.showing_educational_qualifications5 = newVal; // เก็บไฟล์ทั้งก้อน
      } else if (Array.isArray(newVal) && newVal.length > 0) {
        this.formData.showing_educational_qualifications5 = newVal[0]; // เก็บไฟล์แรก (ถ้ามีหลายไฟล์)
      } else {
        this.formData.showing_educational_qualifications5 = null;
      }
      console.log("ข้อมูลไฟล์:", this.formData.showing_educational_qualifications5);
    },
  ...
  methods: {
    ...
    onFileChange() {
      console.log("ไฟล์ที่เลือก:", this.transcript5);
    }
  },
};
</script>
